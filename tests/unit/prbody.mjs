...
describe('Pull request body check', () => {
  ...
  it('should skip if heading hint is invalid', async () => {
    set_env(ENV_VARS.markdownHeading, '234')
    set_env(ENV_VARS.markdownOptionIndex, 0)
    nock_init({"Option 0": true}, [])
    assert(!await prBodyCheck())
  })
  ...
  it('should continue if option is selected (index)', async () => {
    set_env(ENV_VARS.markdownHeading, MARKDOWN_HEADING)
    set_env(ENV_VARS.markdownOptionIndex, 1)
    nock_init({opt0: false, opt1: true}, [])
    assert(await prBodyCheck())
  })
  ...
  it('should skip if option is not selected (index)', async () => {
    set_env(ENV_VARS.markdownHeading, MARKDOWN_HEADING)
    set_env(ENV_VARS.markdownOptionIndex, 1)
    nock_init({opt0: true, opt1: false}, [])
    assert(!await prBodyCheck())
  })

  it('should skip if option is not selected (value)', async () => {
    set_env(ENV_VARS.markdownHeading, MARKDOWN_HEADING)
    set_env(ENV_VARS.markdownOptionValue, 'opt0')
    nock_init({opt0: false, opt1: true}, [])
    assert(!await prBodyCheck())
  })
})